...
public class OutstockDao {
	public ResultSet outstockList(Connection con,PageBean pageBean,OutStock outstock)throws Exception{
		StringBuffer sb=new StringBuffer("select outstockid,goodname,saleprice,outstockdate,outstocknumber,outstocknote,goodid from t_outstock,t_good where t_outstock.goodid=t_good.id");
	...
	public ResultSet outstockList2(Connection con,PageBean pageBean,OutStock outstock)throws Exception{
		StringBuffer sb=new StringBuffer("select goodname,saleprice,outstockdate,outstocknumber,outstocknote,goodid from t_outstock,t_good where t_outstock.goodid=t_good.id");
		if(outstock!=null&&StringUtil.isNotEmpty(outstock.getGoodname())){
			sb.append(" and goodname like '%"+outstock.getGoodname()+"%'");
		}
		if(pageBean!=null){
	...
	public int outstockCount(Connection con,OutStock outstock)throws Exception{
		StringBuffer sb=new StringBuffer("select count(*) as total from t_outstock,t_good where t_outstock.goodid=t_good.id");
	...
	public int outstockAdd(Connection con,OutStock outstock)throws Exception {
		String sql="insert into t_outstock values(null,?,?,?,?,?)";
		PreparedStatement pstmt=con.prepareStatement(sql);
